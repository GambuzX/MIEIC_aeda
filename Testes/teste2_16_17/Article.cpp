...
bool Article::operator<(const Article & art) {
	if (deliverHome && !art.getDeliverHome()) return false;
	if (!deliverHome && art.getDeliverHome()) return true;
	if (!deliverHome && !art.getDeliverHome()) {
		if (presentNumber - art.getPresentNumber() > 2) return false;
		if (art.getPresentNumber() - presentNumber > 2) return true;
	}

	return arrivalNumber < art.getArrivalNumber();
}
