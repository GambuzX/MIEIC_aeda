...
bool Car::operator<(const Car &car) const {
	if (brand == car.brand) return car.price < price;
...
FuelCar::FuelCar(string b, string m, float p, float t, float l) : Car(b, m) {
	tank = t;
	l_100km = l;
	setPrice(p);
}
...
ElectricCar::ElectricCar(string b, string m, float p, float ba, float au) : Car(b, m) {
	battery = ba;
	kWh_100km = au;
	setPrice(p);
}
...
float HybridCar::range() const {
	return getTank()*100/getL_100km() + getBattery()*100/getKWh_100km();
}
